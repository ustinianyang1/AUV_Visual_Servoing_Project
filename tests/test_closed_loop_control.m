...
% Check for NaNs or Infs to ensure reasonability
if any(isnan(x1), 'all') || any(isinf(x1), 'all') || any(isnan(tau_c), 'all') || any(isinf(tau_c), 'all')
    error('Numerical instability detected! NaN or Inf found in states or control signals.');
end
